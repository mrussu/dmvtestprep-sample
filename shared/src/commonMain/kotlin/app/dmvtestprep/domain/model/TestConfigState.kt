...
data class TestConfigState(
    ...
    fun <T> update(testSettingType: TestSettingType, value: T): TestConfigState {
        return when (testMode) {
            PREP -> copy(prepSettings = prepSettings.update(testSettingType, value))
            EXAM -> copy(examSettings = examSettings.update(testSettingType, value))
        }
    }
}